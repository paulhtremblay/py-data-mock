...
def init_old(dirs, db_path, table_name, verbosity = 0):
...
email text,
    ...
    for dir_ in dirs:
        ...
        list_.extend(l)
    if verbosity > 2:
        print(f'list has files has {len(list_)} dicts')
...
def add_old(dirs, start_date, end_date, table_name , db_path,  verbosity = 0):
    list_ = []
    con = sqlite3.connect(db_path)
    cur = con.cursor()
    for dir_ in dirs:
